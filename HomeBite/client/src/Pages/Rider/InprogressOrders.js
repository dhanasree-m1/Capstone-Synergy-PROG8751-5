...
import "./rider.scss"; // Import the SCSS file
...
const CurrentOrders = () => {
  ...
  const fetchOrders = async () => {
    const rider_id=localStorage.getItem("user_id")
    try {
      const response = await fetch("http://localhost:5000/graphql", {
        ...
        body: JSON.stringify({
          query: `
            query {
              getInprogressOrdersRider(rider_id: "${rider_id}") {
                _id
                order_no
                status
                total_amount
      ...
      setOrders(json.data.getInprogressOrdersRider);
    } catch (error) {
    ...
    try {
        const response = await fetch("http://localhost:5000/graphql", {
            ...
            body: JSON.stringify({
                query: `
                  query {
                    getRiderStats(rider_id: "${rider_id}") {
                      todaysOrders
                      todaysEarnings
                      totalOrders
                      totalEarnings
        ...
        const stats = json.data.getRiderStats;
  ...
  return (
    <Row>
      <div className='col-12 mb-3 mt-3'>
        <h2>Welcome back,Rider {localStorage.getItem('uname')}</h2>
      ...
      <div className="col-12 mt-2">
        ...
        <div className="tab-selector">
          <Link to="/rider/orders" className="tab ">Current Orders</Link>
          <Link to="/rider/orders/in-progress" className="tab active">Orders Inprogress</Link>
          <Link to="/rider/orders/completed" className="tab">Order Completed</Link>
                  ...
                  <div className="col-lg-6 text-lg-end">
                    {order.status === "In Progress" ? (
                      <>
                        <Button
                          variant="success"
                          onClick={() => handleAction(order._id, "Completed")} className="small me-3"
                        >
                          Mark Delivered
                        </Button>
                        {/* <Button
